  ...
  <div v-if="irsResult" class="mx-auto">
    ...
    <div class="flex gap-4 p-4 pb-4">
      <Card class="w-[75%]">
        <template #content>
          <div class="align-center px-8">
            <IRSMeter
              :max-tax-credit="irsResult.maxTaxCreditsOverall"
              :tax-credit="irsResult.taxCredits"
            >
            </IRSMeter>
            <div class="pt-[2rem] flex justify-center">
              <DeductionPerCategory
                :max-taxcredits-per-category="irsResult.maxTaxcreditsPerCategory"
              >
              </DeductionPerCategory>
            </div>
          </div>
      ...
      <div class="flex flex-col w-[25%] gap-4">
        <TaxableIncome
          :taxable-income="irsResult.taxableIncome"
          :bracket-level="irsResult.irsBracketLevel"
          :effective-tax="irsResult.effectiveIRSTax"
          :marginal-tax="irsResult.IRSBracketMarginalTax"
        >
        </TaxableIncome>
        <MainInfoResult
          :gross-anual-income="irsResult.grossAnnualIncome"
          :owed-i-r-s="irsResult.IRSDue"
          :withholding-tax="irsResult.withHoldingTax"
          :all-tax-deductions="irsResult.taxCreditsAmount"
        >
        </MainInfoResult>
      </div>
    </div>
